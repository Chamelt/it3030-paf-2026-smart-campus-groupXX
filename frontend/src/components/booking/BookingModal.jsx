                ...
                {items.map(item => {
                    ...
                    return (
                        <div key={item} onClick={() => handleItemClick(item)} style={{
                            ...
                            color: disabled ? 'var(--gray200)' : sel ? 'var(--g700)' : 'var(--gray400)',
...
function computeDisabled(date) {
    ...
    const getDisabledMinutes = (hh, isEndPicker = false) => {
        ...
        if (isEndPicker && h === 22) return new Set(['15', '30', '45'])
        if (!isToday) return new Set()
        if (h < cutoffH) return new Set(MINUTES)
        if (h > cutoffH) return new Set()
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{
                ...
                <DrumColumn items={hourItems} value={hh} onChange={h => onChange(`${h}:${mm}`)} disabledItems={disabledHours} />
                <span style={{ fontSize: 36, fontWeight: 700, color: 'var(--gray300)', lineHeight: 1, flexShrink: 0 }}>:</span>
                <DrumColumn items={MINUTES} value={mm} onChange={m => onChange(`${hh}:${m}`)} disabledItems={disabledMinutes} />
            </div>
        </div>
    )
}
...
export default function BookingModal({ resource, userId, onClose, onSuccess }) {
    ...
    const setStartTime = (v) => {
        setForm(f => ({ ...f, startTime: v, endTime: addThirtyMins(v) }))
    }
    ...
    const validate = () => {
        ...
        // Attendees — required when resource has a capacity
        if (resource.capacity) {
            if (!form.expectedAttendees || form.expectedAttendees === '') {
                e.expectedAttendees = 'Expected attendees is required'
            } else if (Number(form.expectedAttendees) < 1) {
                e.expectedAttendees = 'Must be at least 1 attendee'
            } else if (Number(form.expectedAttendees) > resource.capacity) {
        ...
        if (form.date && form.startTime) {
            ...
            if (bookingStart < twoHoursFromNow) {
                ...
                e.date = `Bookings must be made at least 2 hours in advance. Earliest: ${dateStr} at ${hh}:${mm}.`
            }
        }
    ...
    if (tooLate) {
        ...
        return (
            <div className="fade-in" onClick={e => { if (e.target === e.currentTarget) onClose() }}
                style={{ position: 'fixed', inset: 0, background: 'rgb(0 0 0/0.45)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '2vh 2vw' }}>
                <div className="scale-in" style={{ background: 'var(--white)', borderRadius: 20, boxShadow: '0 32px 64px rgb(0 0 0/0.18)', width: 480, padding: '40px 40px 36px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20, textAlign: 'center' }}>
                    <div style={{ alignSelf: 'flex-end', marginBottom: -8 }}>
                        <button onClick={onClose} style={{ width: 36, height: 36, borderRadius: '50%', border: '1.5px solid var(--border)', background: 'var(--white)', cursor: 'pointer', fontSize: 18, color: 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                            onMouseEnter={e => { e.currentTarget.style.background = '#fef2f2'; e.currentTarget.style.color = '#dc2626' }}
                            onMouseLeave={e => { e.currentTarget.style.background = 'var(--white)'; e.currentTarget.style.color = 'var(--text-muted)' }}>✕</button>
                    </div>
                    <div style={{ fontSize: 56 }}>🌙</div>
                    <div>
                        <span style={{ fontSize: 12, fontWeight: 600, padding: '3px 10px', borderRadius: 999, color: typeC.color, background: typeC.bg, border: `1px solid ${typeC.border}` }}>
                            {RESOURCE_LABEL[resource.type] || resource.type}
                        </span>
                        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 400, marginTop: 10 }}>{resource.name}</h2>
                    </div>
                    <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 12, padding: '18px 22px', width: '100%' }}>
                        <p style={{ fontSize: 16, fontWeight: 600, color: '#92400e', marginBottom: 6 }}>Too late to book today</p>
                        <p style={{ fontSize: 14, color: '#92400e', lineHeight: 1.6 }}>
                            Bookings must be made at least <strong>2 hours in advance</strong>. No slots left for today.
                        </p>
                    </div>
                    <div style={{ width: '100%', textAlign: 'left' }}>
                        <label style={{ display: 'block', fontSize: 14, fontWeight: 600, color: 'var(--text-sec)', marginBottom: 8 }}>Book for a different date</label>
                        <input type="date" defaultValue={tomorrowISO} min={tomorrowISO} onChange={e => set('date', e.target.value)}
                            style={{ width: '100%', padding: '12px 16px', borderRadius: 10, border: '1.5px solid var(--border)', fontSize: 15, fontFamily: 'var(--font-body)', outline: 'none', background: 'var(--white)', cursor: 'pointer' }}
                            onFocus={e => e.target.style.borderColor = 'var(--primary)'}
                            onBlur={e => e.target.style.borderColor = 'var(--border)'} />
                        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 6 }}>Next available: <strong>{tomorrowStr}</strong></p>
                    </div>
                    <button onClick={() => set('date', tomorrowISO)}
                        style={{ width: '100%', padding: '13px', borderRadius: 12, border: 'none', background: 'var(--primary)', color: '#fff', fontSize: 15, fontWeight: 600, fontFamily: 'var(--font-body)', cursor: 'pointer' }}
    ...
    return (
        <div className="fade-in" onClick={e => { if (e.target === e.currentTarget) onClose() }}
            style={{ position: 'fixed', inset: 0, background: 'rgb(0 0 0/0.45)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '2vh 2vw' }}>
            <div className="scale-in" style={{ background: 'var(--white)', borderRadius: 20, boxShadow: '0 32px 64px rgb(0 0 0/0.18)', width: '85vw', maxWidth: 1200, height: '85vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>

                {/* Top bar */}
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '22px 32px', borderBottom: '1px solid var(--border-subtle)', flexShrink: 0 }}>
                    <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 26, fontWeight: 400 }}>Request a Booking</h2>
                    <button onClick={onClose} style={{ width: 40, height: 40, borderRadius: '50%', border: '1.5px solid var(--border)', background: 'var(--white)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, color: 'var(--text-muted)', transition: 'all 150ms' }}
                        ...
                        onMouseLeave={e => { e.currentTarget.style.background = 'var(--white)'; e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.color = 'var(--text-muted)' }}>✕</button>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', flex: 1, overflow: 'hidden' }}>
                    ...
                    <div style={{ borderRight: '1px solid var(--border-subtle)', overflowY: 'auto', padding: '28px', display: 'flex', flexDirection: 'column', gap: 18, background: 'var(--gray50)' }}>
                        <div style={{ height: 160, background: 'linear-gradient(135deg, var(--g50), var(--g100))', borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 72, border: '1px solid var(--g200)' }}>
                            {getIcon(resource.type)}
                        </div>
                        <div>
                            <div style={{ marginBottom: 8 }}>
                                <span style={{ fontSize: 12, fontWeight: 700, padding: '3px 10px', borderRadius: 999, color: typeC.color, background: typeC.bg, border: `1px solid ${typeC.border}`, letterSpacing: '0.04em' }}>
                                    {RESOURCE_LABEL[resource.type] || resource.type}
                                </span>
                            </div>
                            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 400, lineHeight: 1.3 }}>{resource.name}</h3>
                        </div>
                            ...
                            <div>
                                ...
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                                    {resource.features.map(f => (
                                        <span key={f} style={{ fontSize: 12, padding: '4px 10px', borderRadius: 999, background: 'var(--g50)', color: 'var(--g700)', border: '1px solid var(--g200)' }}>{f.replace(/_/g, ' ')}</span>
                                    ))}
                                </div>
                            </div>
                        )}
                    ...
                    <div style={{ overflowY: 'auto', padding: '28px 36px', display: 'flex', flexDirection: 'column', gap: 22 }}>
                        ...
                        {/* Date */}
                        <div>
                            <label style={{ display: 'block', fontSize: 14, fontWeight: 600, color: 'var(--text-sec)', marginBottom: 8 }}>Date *</label>
                            <input type="date" value={form.date} min={todayISO()} onChange={e => set('date', e.target.value)}
                                style={{ width: '100%', padding: '12px 16px', borderRadius: 10, border: `1.5px solid ${errors.date ? '#dc2626' : 'var(--border)'}`, fontSize: 15, fontFamily: 'var(--font-body)', outline: 'none', background: 'var(--white)' }}
                                onFocus={e => e.target.style.borderColor = 'var(--primary)'}
                        ...
                        {/* Availability */}
                        {checking && (
                            <div style={{ padding: '11px 16px', borderRadius: 10, background: 'var(--gray50)', border: '1px solid var(--border)', fontSize: 14, color: 'var(--text-muted)' }}>Checking availability…</div>
                        )}
                        {!checking && avail && (
                            <div style={{ padding: '11px 16px', borderRadius: 10, fontSize: 14, fontWeight: 500, background: avail.available ? 'var(--g50)' : '#fef2f2', color: avail.available ? 'var(--g700)' : '#dc2626', border: `1px solid ${avail.available ? 'var(--g200)' : '#fecaca'}` }}>
                        ...
                        {/* Purpose */}
                        <div>
                            <label style={{ display: 'block', fontSize: 14, fontWeight: 600, color: 'var(--text-sec)', marginBottom: 8 }}>Purpose *</label>
                            <textarea rows={3} value={form.purpose} onChange={e => set('purpose', e.target.value)}
                                ...
                                style={{ width: '100%', padding: '12px 16px', borderRadius: 10, border: `1.5px solid ${errors.purpose ? '#dc2626' : 'var(--border)'}`, fontSize: 15, fontFamily: 'var(--font-body)', outline: 'none', resize: 'vertical', background: 'var(--white)' }}
                                onFocus={e => e.target.style.borderColor = 'var(--primary)'}
                        ...
                        {/* Attendees — required when resource has capacity */}
                        {resource.capacity && (
                            <div>
                                    ...
                                    return (
                                        <>
                                            <input
                                                ...
                                                style={{ width: '100%', padding: '12px 16px', borderRadius: 10, border: `1.5px solid ${borderColor}`, fontSize: 15, fontFamily: 'var(--font-body)', outline: 'none', background: overCapacity ? '#fef2f2' : 'var(--white)', transition: 'border-color 150ms, background 150ms' }}
                                            ...
                                            {overCapacity ? (
                                                <p style={{ fontSize: 13, color: '#dc2626', marginTop: 6, fontWeight: 500 }}>
                                                    ✕ {form.expectedAttendees} exceeds capacity of {resource.capacity}
                                                </p>
                                            ) : errors.expectedAttendees ? (
                                                <p style={{ fontSize: 13, color: '#dc2626', marginTop: 5 }}>{errors.expectedAttendees}</p>
                                            ) : (
                        ...
                        <div style={{ padding: '16px 18px', background: 'var(--gray50)', borderRadius: 12, border: '1px solid var(--border)' }}>
                            <label style={{ display: 'flex', alignItems: 'flex-start', gap: 14, cursor: 'pointer' }}>
                                <div onClick={() => set('isPriority', !form.isPriority)} style={{ width: 44, height: 24, borderRadius: 999, flexShrink: 0, marginTop: 2, background: form.isPriority ? 'var(--primary)' : 'var(--gray300)', position: 'relative', transition: 'background 200ms' }}>
                                    <div style={{ position: 'absolute', top: 3, width: 18, height: 18, left: form.isPriority ? 23 : 3, borderRadius: '50%', background: '#fff', boxShadow: '0 1px 3px rgb(0 0 0/0.2)', transition: 'left 200ms' }} />
                                </div>
                                <div>
                                    <span style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)' }}>⚡ Priority Booking</span>
                                    <span style={{ display: 'block', fontSize: 13, color: 'var(--text-muted)', marginTop: 2 }}>Mark if this booking requires urgent or special consideration</span>
                                </div>
                            </label>
                            {form.isPriority && (
                                <div style={{ marginTop: 14 }}>
                                    <textarea rows={2} value={form.priorityReason} onChange={e => set('priorityReason', e.target.value)}
                                        placeholder="Explain why this needs priority status"
                                        style={{ width: '100%', padding: '10px 14px', borderRadius: 8, border: `1.5px solid ${errors.priorityReason ? '#dc2626' : 'var(--g300)'}`, fontSize: 14, fontFamily: 'var(--font-body)', outline: 'none', resize: 'vertical', background: 'var(--white)' }} />
                        ...
                        {errors._global && (
                            <div style={{ padding: '12px 16px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 10, fontSize: 14, color: '#dc2626' }}>{errors._global}</div>
                        )}
                        ...
                        <button onClick={handleSubmit}
                            ...
                            style={{ padding: '14px 32px', borderRadius: 12, border: 'none', background: (loading || (avail && !avail.available)) ? 'var(--gray300)' : 'var(--primary)', color: '#fff', fontSize: 16, fontWeight: 600, fontFamily: 'var(--font-body)', cursor: (loading || (avail && !avail.available)) ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, transition: 'filter 150ms' }}
                        ...
                        {avail && !avail.available && (
                            <p style={{ fontSize: 13, color: '#dc2626', textAlign: 'center' }}>This slot is not available. Please choose a different time.</p>
                        )}
                    </div>
                </div>
            </div>
