...
public interface BookingRepository extends JpaRepository<Booking, UUID> {
        ...
        // ------------------------------------------------------------------
        @Query("""
                        SELECT FUNCTION('date_part', 'hour', b.startTime), COUNT(b)
                        FROM Booking b
                        WHERE b.status = 'APPROVED'
                        AND (:resourceId IS NULL OR b.resource.resourceId = :resourceId)
                        GROUP BY FUNCTION('date_part', 'hour', b.startTime)
                        ORDER BY FUNCTION('date_part', 'hour', b.startTime)
                        """)
